...
PYBIND11_MODULE(pygsound, ps)
{
    ...
    ps.def( "createbox", py::overload_cast<float, float, float, std::vector<float>, float>(&SoundMesh::createBox),
      ...
      py::class_< Scene, std::shared_ptr< Scene > >( ps, "Scene" )
            ...
            .def( "getPathData", py::overload_cast<std::vector<SoundSource>&, std::vector<Listener>&, Context&, float, size_t>(&Scene::getPathData),
                  "A function to get detailed path data based on pre-defined sources and listeners",
                  py::arg("_sources"), py::arg("_listeners"), py::arg("_context"),
                  py::arg("energy_percentage") = 100.0f, py::arg("max_rays") = 0)
            .def( "getPathData", py::overload_cast<std::vector<std::vector<float>>&, std::vector<std::vector<float>>&, Context&, float, float, float, float, size_t>(&Scene::getPathData),
